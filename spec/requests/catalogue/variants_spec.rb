...
RSpec.describe "Catalogue::Variants", type: :request do
  ...
  let(:product) { supplier.products.create(accounting_code_id: accounting_code.id, title: 'my product', description: 'my product description', sku_code: '987999', publish: true, markup: '50', cost_price: 10099) }
  ...
  let(:good_params) { { variant: { quantity: 0, product_attributes_variants_attributes: { "0": { product_attribute_id: attr_1.id }, "1": { product_attribute_id: attr_2.id } } } } }
  ...
  describe 'POST /catalogue/supplier/:id/product/:id/variants' do
    context 'with good params' do
      ...
      it 'creates a variant that is small and red' do
        post catalogue_supplier_product_variants_path(supplier.id, product.id, params: good_params)
        expect(product.reload.variants.first.size).to eq('small')
        expect(product.reload.variants.first.colour).to eq('red')
      end
    end
  end
end
